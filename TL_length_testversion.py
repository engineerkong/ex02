...
class TL:
    ...
    def _setup_evalenv(self, gravity):
        ...
        """
        if gravity == 1.0:
            self.env_eval = gym.make("Pendulum-v1")
        elif gravity == 0.01:
            self.env_eval.l = 0.01
            print(f"length:{self.env_eval.l}")
        elif gravity == 100.0:
            self.env_eval.l = 100.0
            print(f"length:{self.env_eval.l}")
    ...
    def plot(self):
        ...
        for i in range(len(self.gravity_list)):
            ...
            plt_list.append(L)
            legend_list.append(f"Evaluate with length {self.gravity_list[i]}")
...
if __name__ == "__main__":
    process = TL(gravity_list=[1.0, 0.01, 100.0], seeds=[2, 6, 1])
