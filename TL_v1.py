...
class TL:
    ...
    def learn(self):
        ...
        for k in range(len(self.seeds)):
            ...
            self.model = DDPG("MlpPolicy", self.env_train, learning_rate=self.alpha, gamma=self.gamma, action_noise=action_noise, verbose=1)
            for epoch in range(self.epochs):
                print(f"epoch:{epoch}")
                self.model = self.train(self.model)
                reward_no = self.eval(self.model,gravity=None, seed=self.seeds[k])
                reward_g = self.eval(self.model,gravity=self.gravity_change, seed=self.seeds[k])
